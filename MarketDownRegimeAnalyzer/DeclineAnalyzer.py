...
class DeclineAnalyzer:
    ...
    def process_data(self, ema_period=20, std_multiplier=2.0):
        ...
        self.data["EMA"] = (
            self.data["Pct_Change"].ewm(span=ema_period, adjust=False).mean()
        )
        self.data["Rolling_StdDev"] = (
            self.data["Pct_Change"].rolling(window=ema_period).std()
        )
    ...
    def calculate_threshold(self, index, std_multiplier):
        ...
        if not np.isnan(self.data["EMA"].iloc[index]):
            threshold = (
                self.data["EMA"].iloc[index]
                - std_multiplier * self.data["Rolling_StdDev"].iloc[index]
            )
        else:
            threshold = np.nan
        return threshold

    def update_peak_and_decline(
        self,
        i,
        peak_index,
        peak_value,
        cumulative_decline,
        consecutive_above_peak,
        std_multiplier,
    ):
        """
    ...
    def load_and_process_data(self, ema_period=20, std_multiplier=2.0):
        ...
        for i in range(1, len(self.data)):
            peak_index, peak_value, cumulative_decline, consecutive_above_peak = (
                self.update_peak_and_decline(
                    i,
                    peak_index,
                    peak_value,
                    cumulative_decline,
    ...
    def save_data(self):
        ...
        """
        self.data.to_csv("../MarketDownRegimeAnalyzer/data.csv")
