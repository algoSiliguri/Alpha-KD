...
class MonteCarlo:
  ...
  def generate_path(self, number_observation=1000):
      ...
      # Compute the candles length in seconds based on the frequency
      if freq.startswith('D'):
          time_index = [86400] * len(df)  # 1 day in seconds
      elif freq.startswith('W'):
          time_index = [604800] * len(df)  # 1 week in seconds
      elif freq.startswith('M'):
          time_index = [2592000] * len(df)  # 1 month (approx 30 days) in seconds
      else:
      ...
      # Create columns with the length of the bar and how much time you need to touch the low and high time
