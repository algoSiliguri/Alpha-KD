...
class CCI_Flag:
    def __init__(self, input_dir, output_file, cci_window, atr_window, constant=0.015):
        ...
        for file_name in os.listdir(self.input_dir):
            if file_name.endswith(".csv"):
                ...
                if cci_previous < -100 < cci_current:
                    ...
                    flagged_stocks.append(stock_name)

        # After collecting flagged stocks
        if flagged_stocks:
            # Create a DataFrame from the list of flagged stocks
            result_df = pd.DataFrame(flagged_stocks)

            # Write the DataFrame to the output CSV file without the header
            result_df.to_csv(self.output_file, index=False, header=False)
...
if __name__ == "__main__":
    ...
    cci_flag = CCI_Flag(input_dir=input_dir, output_file=output_file, cci_window=15, atr_window=15, constant=0.015)
