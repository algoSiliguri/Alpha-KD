...
class CCI_Flag:
    def __init__(self, input_dir, output_file, cci_window, atr_window, constant=0.015, atr_multiplier=1.5, take_profit=0.01,):
    ...
    def flag_stocks(self):
        ...
        for file_name in os.listdir(self.input_dir):
            if file_name.endswith(".csv"):
                ...
                if cci_previous < -100 < cci_current:
                    ...
                    flagged_stocks.append((stock_name, last_day_close, self.take_profit, trailing_stop))
        ...
        if flagged_stocks:
            ...
            result_df = pd.DataFrame(flagged_stocks,
                                     columns=['Stock Name', 'Last Day Close', 'Take Profit', 'Trailing Stop'])
