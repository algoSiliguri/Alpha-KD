...
class TradingStrategy:
    ...
    def check_gap_up(self, i):
        ...
        try:
            if i < 3:
                return (
                    False  # Do not generate any buy/sell signal for the first 2 hours
                )
                ...
                if i == 3:
                    ...
                    if (
                        self.data["Close"].iloc[i - 1] < self.data["VWAP"].iloc[i - 1]
    ...
    def check_gap_down(self, i, stop_loss):
        ...
        try:
            ...
            if (
                self.data["Close"].iloc[i] >= stop_loss
                and self.data["Close"].iloc[i] >= self.data["VWAP"].iloc[i]
            ):
                return True
            return False
    ...
    def create_signals(self, df_intraday_30min):
        ...
        try:
            ...
            if self.gap is None:
                self.gap = (
                    (df_intraday_30min["Open"].iloc[0] - self.previous_day_close)
            ...
            for i in range(len(df_intraday_30min)):
                if self.gap > 3:
                    ...
                    if self.check_gap_down(
                        i, self.previous_day_close * 0.97
                    ):  # Example stop loss at 3% below previous close
                        buy = True
                        break  # Exit loop after the first buy signal
