...
while True:
    ...
    if datetime.now().strftime("%H:%M:%S") in timeframe_condition:
        ...
        # Create the signals
        buy, sell, pct_tp, pct_sl = li_2023_02_RsiSmaAtr(
            symbol, timeframe[0], 60, 120, 20
        )
        print(pct_tp, pct_sl)
        ...
        # Generally you run several assets in the same time, so we put sleep to avoid to do again the
