...
class ParameterTuner:
  ...
  def tune_threshold(self, signal, wavelet, level, thresholds, mode='soft'):
      ...
      for t in thresholds:
          denoised_signal = self.denoiser.wavelet_denoise(signal, wavelet, level, t, mode)
  ...
  def tune_wavelet(self, signal, wavelets, level, threshold, mode='soft'):
      ...
      for wavelet in wavelets:
          denoised_signal = self.denoiser.wavelet_denoise(signal, wavelet, level, threshold, mode)
          mse, snr = self.evaluator.evaluate_performance(signal, denoised_signal)
          if mse < best_mse:
              best_mse = mse
              best_snr = snr
